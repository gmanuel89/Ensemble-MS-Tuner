...
functions_mass_spectrometry <- function() {
  ...
  ################## FUNCTIONS - MASS SPECTROMETRY 2017.11.09 ################
      ...
      if (low_intensity_peak_removal_threshold_method == "element-wise" || low_intensity_peak_removal_threshold_method == "spectrum-wise") {
        ...
        if (isMassPeaksList(peaks)) {
          ...
          if ((is.logical(allow_parallelization) && allow_parallelization == TRUE) || (is.character(allow_parallelization) && allow_parallelization == "lapply")) {
            ...
            cpu_thread_number <- detectCores(all.tests = TRUE, logical = FALSE)
  ...
  feature_selection <<- function(training_set, feature_selection_method = "ANOVA", features_to_select = 20, selection_method = "pls", selection_metric = "Kappa", correlation_method = "pearson", correlation_threshold = 0.75, auc_threshold = 0.7, cv_repeats_control = 5, k_fold_cv_control = 10, discriminant_attribute = "Class", non_features = c("Sample", "Class", "THY"), seed = NULL, automatically_select_features = FALSE, generate_plots = TRUE, preprocessing = c("center","scale"), allow_parallelization = FALSE, feature_reranking = FALSE) {
    ...
    if (feature_selection_method == "importance") {
      ...
      model_control <- trainControl(method = "repeatedcv", number = k_fold_cv_control, repeats = cv_repeats_control, classProbs = TRUE)
...
ensemble_ms_tuner <- function() {
  ...
  ############## INSTALL AND LOAD THE REQUIRED PACKAGES
  install_and_load_required_packages(c("tcltk", "parallel", "caret", "stats", "pROC", "nnet", "e1071", "kernlab", "MASS", "klaR", "pls", "randomForest", "lda", "SparseM", "stringi", "doParallel"), repository = "http://cran.mirror.garr.it/mirrors/CRAN/", update_packages = TRUE, print_messages = TRUE)
  ...
  outcome_list_function <- function() {
    ...
    if (!is.null(class_list) && !is.null(peaklist)) {
      ...
      submit_outcome_list_function <- function() {
        ...
        tkgrid(outcome_list_value_label, row = 7, column = 4, padx = c(10, 10), pady = c(10, 10))
        # Escape the function
      ...
      outcome_list_label <- tklabel(outcome_list_window, text = "Outcome list\n(separated by comma)\n(b = benign ; m = malignant ; o = other)", font = label_font, bg = "white", width = 40, justify = "center")
      outcome_list_entry <- tkentry(outcome_list_window, width = 20, textvariable = outcome_list_input, font = entry_font, bg = "white", width = 30, justify = "center")
  ...
  file_type_export_matrix_choice <- function() {
    ...
    tkgrid(file_type_export_matrix_value_label, row = 5, column = 5)
    # Raise the focus on the main window
    tkraise(window)
  }
  ...
  file_import_function <- function() {
    ...
    if (filepath_import != "") {
      ...
      try({
        ...
        try({
          peaklist$Class <- make.names(as.character(peaklist$Class), unique = FALSE)
        ...
        non_features <- select.list(feature_vector, title = "Demographical features", multiple = TRUE, preselect = c("Sample", "Class"))
        ...
        if (discriminant_attribute == "NONE" || discriminant_attribute == "") {
          ...
          peaklist[, discriminant_attribute] <- make.names(as.character(peaklist[, discriminant_attribute]))
          class_list <- levels(as.factor(peaklist[, discriminant_attribute]))
        }
        ##### Force peaks
        # Peaks to retain
  ...
  run_ensemble_ms_tuner_function <- function() {
    ...
    if (!is.null(filepath_import) && !is.null(outcome_list)) {
      ...
      model_ensemble_tuner <- model_ensemble_embedded_fs(training_set = peaklist, features_to_select = features_to_select, model_tuning = model_tuning, selection_metric = selection_metric, discriminant_attribute = discriminant_attribute, non_features = non_features, seed = 12345, automatically_select_features = automatically_select_features, generate_plots = generate_plots, cv_repeats_control = cv_repeats_control, k_fold_cv_control = k_fold_cv_control, preprocessing = preprocessing, allow_parallelization = allow_parallelization, feature_reranking = feature_reranking, try_combination_of_parameters = try_combination_of_parameters, outcome_list = outcome_list, progress_bar = "tcltk", test_set = NULL)
      ...
      if (file_type_export_matrix == "csv") {
        ...
        writeWorksheetToFile(file = paste0("Model performance matrix", ".", file_type_export_matrix), data = model_performance_matrix, clearSheets = TRUE, sheet = "Model performances")
      }
      # Dump the model common features matrix file
      if (file_type_export_matrix == "csv") {
        write.csv(common_features_matrix, file = paste0("Common features matrix", ".", file_type_export_matrix), row.names = FALSE)
      } else if (file_type_export_matrix == "xls" || file_type_export_matrix == "xlsx") {
        writeWorksheetToFile(file = paste0("Common features matrix", ".", file_type_export_matrix), data = common_features_matrix, clearSheets = TRUE, sheet = "Common features")
      }
      # Dump the model features matrix file
      if (file_type_export_matrix == "csv") {
        write.csv(feature_list$model_features_matrix, file = paste0("Model features matrix", ".", file_type_export_matrix), row.names = FALSE)
      } else if (file_type_export_matrix == "xls" || file_type_export_matrix == "xlsx") {
        writeWorksheetToFile(file = paste0("Model features matrix", ".", file_type_export_matrix), data = feature_list$model_features_matrix, clearSheets = TRUE, sheet = "Model features")
      }
      # Dump the list of major features matrix file
      if (file_type_export_matrix == "csv") {
        write.csv(as.matrix(cbind(feature_list$model_features)), file = paste0("Model features", ".", file_type_export_matrix), row.names = FALSE)
      } else if (file_type_export_matrix == "xls" || file_type_export_matrix == "xlsx") {
        writeWorksheetToFile(file = paste0("Model features", ".", file_type_export_matrix), data = feature_list$model_features, clearSheets = TRUE, sheet = "Model features")
      }
      # Dump the list of models matrix file
      model_list_matrix <- as.matrix(cbind(names(model_list)))
      colnames(model_list_matrix) <- "Models"
      if (file_type_export_matrix == "csv") {
        write.csv(model_list_matrix, file = paste0("Model list", ".", file_type_export_matrix), row.names = FALSE)
      } else if (file_type_export_matrix == "xls" || file_type_export_matrix == "xlsx") {
  ...
  tkgrid(outcome_list_entry, row = 7, column = 3, padx = c(10, 10), pady = c(10, 10))
  tkgrid(outcome_list_value_label, row = 7, column = 4, padx = c(10, 10), pady = c(10, 10))
  ...
  tkgrid(file_type_export_matrix_entry, row = 5, column = 4, padx = c(10, 10), pady = c(10, 10))
  tkgrid(file_type_export_matrix_value_label, row = 5, column = 5, padx = c(10, 10), pady = c(10, 10))
  tkgrid(browse_output_button, row = 6, column = 5, padx = c(10, 10), pady = c(10, 10))
  tkgrid(file_import_button, row = 7, column = 2, padx = c(10, 10), pady = c(10, 10))
  tkgrid(run_ensemble_ms_tuner_function_button, row = 7, column = 5, padx = c(10, 10), pady = c(10, 10))
  tkgrid(end_session_button, row = 7, column = 6, padx = c(10, 10), pady = c(10, 10))
